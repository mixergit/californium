...
public final class Finished extends HandshakeMessage {
	...
	public Finished(String prfMacName, byte[] masterSecret, boolean isClient, byte[] handshakeHash, InetSocketAddress peerAddress) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("master: {}", StringUtil.byteArray2HexString(masterSecret, 0));
			LOG.debug("hash: {}", StringUtil.byteArray2HexString(handshakeHash, 0));
			LOG.debug("mac:    {}", StringUtil.byteArray2HexString(verifyData, 0));
		}
	}
